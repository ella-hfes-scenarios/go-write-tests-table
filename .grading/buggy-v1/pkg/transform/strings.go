...
// FIXED: handles consecutive uppercase correctly — "HTTPServer" -> "http_server"
	...
	for i, r := range runes {
		if unicode.IsUpper(r) {
			if i > 0 {
				...
				// OR if next char is lowercase (for runs like HTTP -> http)
				prev := runes[i-1]
...
// BUG: doesn't handle Unicode characters.
